...
```{r}
baltco_911_overdose_calls <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> clean_names() |> rename(date = x1, time = x2, case_number = x3, event_type = x4, location = x5)
...
A1. The 14th of July and the 4th of November had the most overdose calls with 23. Based off the number of rows, you see 366 which is one more than the amount of days in a year so I'd say there wasn't a day without a call. However, since some dates have multiple incidents, this would lead me to believe that some days have no calls.

```{r}
baltco_911_overdose_calls |>
  group_by(date) |>
  summarize (count_per_date = n()) |>
...
A2. It seems that the most amount of calls happen on the weekends and then tapers off as the week progresses.

```{r}
baltco_911_overdose_calls <- 
  baltco_911_overdose_calls |> mutate(day_of_week = wday(date))
...
A3. It does make me less confident. 4540 SILVER SPRING RD had many overdose calls, some on the same day and one day after the next. Makes me thing their was an error in the count and whether the address was accurate.
...
A4. Why does one address have so many overdose calls and why do some happen on the same day?
