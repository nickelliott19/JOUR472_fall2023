...
census_api_key("933ecfdabb755c55d48072248a248184a072f293", install = TRUE)
```
...
**Q1** What is the Maryland jurisdiction with the lowest median age, according to the ACS 2021 5-year estimates? You will need to find the variable for median age first (search for it in the 'concept' column at the data frame we just created). **A1** Baltimore City has the lowest median age.

```{r}
md_median_age <- get_acs(geography="county", variables = "B01002_001", state='MD', year=2021)

md_median_age |> arrange(estimate)
```

**Q2** Which Maryland counties have a median age of at least 50? You can use the dataframe you just made to answer this, but you must write code to display only those counties with a median age of at least 50. **A2** Talbot and Worcester County's have a median age of at least 50.
...
**A3** 20877 and 20852 have the highest rate of overdose calls and 20832 and 20707 have the lowest.
...
moco_911_calls_per_zip <- moco_911_calls |>
  group_by(zip) |> summarize(count = n()) 
...
**A4** The zip code 20877 had the highest rate of overdose calls and has a high Hispanic population with around 77,000 median household income. 20707 has a predominantly black population with around a 91,000 median household income. There could be a story comparing overdose calls to race and income.
...
**A5** I do get the same zip codes and that does give me confidence since the moe doesn't seem to be big.

```{r}
moco_join_pop_calls <- moco_join_pop_calls |> 
  mutate(min=(estimate-moe), max=(estimate+moe)) |>
  mutate(rate_per_1k_min = (count/min)*1000) |> arrange(desc(rate_per_1k_min))|>
  mutate(rate_per_1k_max = (count/max)*1000) |> arrange(desc(rate_per_1k_max))
