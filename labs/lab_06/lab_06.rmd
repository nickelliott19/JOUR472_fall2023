...
# You can do clean_names () or rename
carroll_911<- carroll_911 |> rename (date = Date, location = Location )

cecil_allegany_carroll_911 <- bind_rows(list(cecil_911, allegany_911, carroll_911))
View(cecil_allegany_carroll_911)
...
A1.Cecil has the most, Carroll has the second most, and Allegany has the least 911 calls.

```{r}
cecil_allegany_carroll_911 |>
  group_by(county) |>
  summarize(count = n()) |>
  arrange(desc(county))
```
...
A2. 314 Grove Neck Rd Earleville, MD in Cecil county had the most calls with 15
...
A3.21217 is in north west Baltimore and encompasses Druid Hills, Madison Park, and Penn North. 21223 is in west Baltimore encompassing Lexington and Carrollton Ridge. Downtown in 21201 
...
A4:It seems that the data suggests that those communities with more renters tend to have more calls. 

```{r}
baltimore_911_with_demographics |>
  group_by(neighborhood, pct_rent, pct_homeowner) |>
  summarize(
    number_of_calls = n())|>
