...
media_expenses <- md_expenses |>
  # Filters in just payees that relate to media
...
**A2.** The candidate committees of Ashwani Jain, Peter Franchot, Jon Baron and John King ran for governor. The non-candidates were the WFP PAC who are fighting against political corruption and working for working class people and the Casa in Action PAC is supporting miniorities and mobilizing voters. Billy Boniface ran for Harford County Council and Eve Shuman ran for PG County Council. Ashwani Jain's ads have all been removed for not following certain policies.
...
**A3.** Wes Moore is missing from the list. The Wes Moore campaign did run ads according to Facebook's ad library. The Win Company is the largest recipient.

```{r}
media_expenses_clean |> 
  filter(expense_toward == 'WESTLEY MOORE - SUPPORT') |>
  group_by(payee_name) |>
  summarize(total_amount = sum(amount)) |>
  arrange(desc(total_amount))
...
**A4.** The list is completely different and I see that these committees spent very little on Facebook ads.

```{r}
committees_online_expenses<- media_expenses_clean |>
    filter(expense_purpose == 'Online Advertising') |>
  group_by(committee_name) |>
  summarize(total_amount = sum(amount)) |>
  arrange(desc(total_amount))

committees_online_expenses
...
**A5** It seems that Facebook ads aren't very effective and the mass amount of money towards advertisements has been spent in other ways. The story could be about how candidates are moving away from Facebook ads but we don't know if that's a rational decision or a popular one. 
