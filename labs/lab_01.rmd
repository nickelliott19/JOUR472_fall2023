...
```
I'm not surprise by the high average number of seats for engineering and computer science, but I am suprised by "Air Science" being that high. Especially since I don't really know what that means.
...
The answer would be easier to see if the code could exclude "NA" as an instructor as the first few pages don't show an actual instructor.
...
umd_courses |>
  ...
  arrange(desc(count_classes))|>
  na.exclude(instructors)
```
While I was able to exclude the NA values, it didn't exclude "Instructors: TBA" since it's technically a value but I should've been excluded because it means the same thing.
