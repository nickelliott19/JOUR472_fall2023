...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. https://datawrapper.dwcdn.net/ON0yT/1/ 
...
md_gov_county <- md_gov_county |>
  mutate(total_votes = cox + moore + lashar + wallace + harding, pct_moore = (moore/total_votes)*100)
...
**Answer** Describe the changes from the previous map. I see just Maryland with different shaded counties regarding the density of Moore voters in each. 
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map. Now the percentage numbers don't go forever and are just to the first decimal point.
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer** Put the URL of your map here. https://datawrapper.dwcdn.net/pSs8N/2/
