...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Yes, other than that 2020 says "total" instead of "TOTAL" in that column.
...
```{r}
# bind_rows with list
county_voters_2022 <- county_voters_2022 |> rename (TOTAL = Total)
```

### Task 4: Asking a question

**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** The population increased but democrat voters increased quite substantially.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 and there should be 24 because they are 24 counties.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It has the race demographics of counties and also adults percentages for those races. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Princes George's County has the highest percent of black people over 18 and also the highest percentage of democrat voters. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard and Montgomery County have a very high percentage of asian adults compared to the rest of the state and also there doesn't seem to be a trend for asians and how they vote.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** ZCTA's are a chr column because its not a number we do math on it is instead representing an area thus not a dbl column.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** A ZCTA is a character not a dbl
