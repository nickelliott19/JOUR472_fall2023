...
**Answer** It's important because it holds the result and ensures that we are just looking at the JOUR classes. 
...
**Answer:** The expenditure column now has the date configurations as y-m-d instead of m/d/y
...
**Answer** The values aren't in any particular order meaning the total amount flucuates randomly month to month.
...
```{r}
general_22 <- read_csv('md_gov_county.csv')
```
...
**Answer** The code is making a new column that arranges the total votes for each candidate and then how much a county voted for moore based on percentage
...
**Answer**The answer changed because this data set arranged the county's with the highest percentage voting for moore to the least.
...
**Answer** It's interesting in Maryland because it's known as a very democratic state and a county only voting 22% for Moore and seeing the rest that didn't vote heavily for him is interesting.
...
**Answer** str_to_lower: Converts a character column to all lowercase
str_to_title: Converts a character column to title case which means only the first letter of each word is capitalized
...
**Answer** The statment is creating a category that just includes the state's initials. But, its finding the initials by detecting the state's full name in the address. 
...
**Answer** Yes it does because it orders each state descending and gives the total expenditures of each. Makes sense that Maryland is at the top since it's Maryland expenditures.
...
1. Which department has the most 300+ level classes?

2.  Which county has the highest percentage of total votes?

3.  Which county could be seen as most democrat?
