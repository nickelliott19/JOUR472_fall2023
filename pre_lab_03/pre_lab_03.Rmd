...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer: It says it has parsing issues and I'm not sure what exactly the instructions are.**

```{r}
payments <- read_csv("State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max=10)
```

### Task 3: Reload data

**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: No it doesn't.**

```{r}
payments <- read_csv("State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max=373564)
```

### Task 4: Examine the data with glimpse

**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The "Vendor Name" is a chr data type, "Amount" is a dbl data type and "Date" is a chr data type.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: There are several "NA" categories.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The smallest amount is 60\$ which is a relatively small number to be loaned so that's odd.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: It made everything lowercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: Seems like 2 duplicates is the height which makes me think its not a mistake because if it was unintentional then the number would most likely be less consistent.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: Now it lists the zip codes in only 5 digits.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? **Answer: Yes it makes sense to me because that zip code is the most "popping" spot in all of Baltimore which is right at the inner harbor and tends to be where most tourists would go**
